...
Graphs
```{r}
print(ggplot(data=xdata, aes(x=method, y=n_sightings,fill=method,pattern=method)) +
  geom_boxplot_pattern(pattern_fill="black",pattern_color="black",outlier.size = 2.3)+
    ylab("No. of Independent Observations")+
  ylim(0,230)+
           geom_jitter(color="black", size=1, alpha=0.9) +
    scale_x_discrete(labels = c("cam" = "Camera Trapping", "walk1" = "Transect Sampling 1", "walk2" = "Transect Sampling 2"))+
       theme_classic()+
  ...
  guides(fill = guide_legend(title = NULL))+
    geom_signif(stat="signif",position="identity",comparisons = list(c("cam", "walk2")),map_signif_level=TRUE,color = "black",annotations="***",y_position=210,textsize = 8.5)+
  scale_pattern_manual(values = c(cam = "none",walk1= "none",walk2="stripe")) +
    geom_signif(stat="signif",position="identity",comparisons = list(c("cam", "walk1")),map_signif_level=TRUE,color = "black",annotations="***",y_position=190,textsize=8.5)+  scale_fill_manual(labels = custom_labels, values = c("cam"="black","walk1"="red","walk2"="red"))+
...
Graphs:
```{r}
custom_labels <- c("Camera Trapping", "Transect Sampling (1 Obs.)", "Transect Sampling (2 Obs.)")

print(ggplot(data=xdata, aes(x=method, y=n_species_detected,fill=method,pattern=method)) +
  geom_boxplot_pattern(pattern_color="black",pattern_fill="black",outlier.size = 2.3)+
  # xlab("Survey method")+
    ylab("No. of Species")+
    theme_classic()+
    geom_jitter(color="black", size=1, alpha=0.9) +
    scale_x_discrete(labels = c("cam" = "Camera Trapping", "walk1" = "Transect Survey 1", "walk2" = "Transect Survey 2"))+
  ...
  guides(fill = guide_legend(title = NULL))+
    ylim(0,15)+
    geom_signif(stat="signif",position="identity",comparisons = list(c("cam", "walk2")),map_signif_level=TRUE,color = "black",annotations="***",y_position=13.5,textsize = 8.5)+
    geom_signif(stat="signif",position="identity",comparisons = list(c("cam", "walk1")),map_signif_level=TRUE,color = "black",annotations="***",textsize=8.5)+
  scale_pattern_manual(values = c("none", "none","stripe")) +
  scale_fill_manual(labels = custom_labels, values = c("cam"="black","walk1"="red","walk2"="red"))+theme(legend.position = "none"))
...
```{r}
print(ggplot(data=xdata_naomit, aes(x=method, y=mean_weight_animal_detected,fill=method,pattern=method)) +
  geom_boxplot_pattern(pattern_fill="black",pattern_colour="black",outlier.size = 2.3)+
   xlab("Survey Method")+
    ylab("Weight of Animals Detected (kg)")+
  theme_classic()+
    ylim(0,1300)+
    theme(axis.title.x = element_blank(),axis.title.y = element_text(size = 30), axis.text.y = element_text(color = "black"),axis.text.x = element_text(color = "black"),text=element_text(size=30, family="serif"))+
     scale_x_discrete(labels = c("cam" = "Camera Trapping", "walk1" = "Transect Sampling (1 Obs.)", "walk2" = "Transect Sampling (2 Obs.)"))+
         geom_jitter(color="black", size=1, alpha=0.9) +
  scale_pattern_manual(values=c("none","none","stripe"))+
  scale_fill_manual(values = c("cam"="black","walk1"="red","walk2"="red"))+
    geom_signif(stat="signif",position="identity",comparisons = list(c("walk1", "walk2")),map_signif_level=TRUE,annotations="**",color="black",textsize=8.5)+theme(legend.position = "none"))
ggsave("m_weight.png",height=8,width = 16)

png("m_weight_precip.png",width=600, height=350)
  print(ggplot(xdata_naomit, aes(x = precip, y = mean_weight_animal_detected)) + 
  geom_point() +
    xlab("Precipitation (mm)")+
    ylab("Average weight of animals detected (kg)")+
